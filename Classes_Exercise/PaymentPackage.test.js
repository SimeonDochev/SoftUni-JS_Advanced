...
describe('Payment Package', () => {
    ...
    it('name setter throws error when not given string', () => {
        expect(() => {
            (testPackage.name = 5).to.throw('Name must be a non-empty string');
        });
    });
    ...
    it('name setter sets the new name correctly', () => {
        expect(testPackage.name).to.equal('test');
        testPackage.name = 'new';
        expect(testPackage.name).to.equal('new');
    });
    ...
    it('value setter sets the new value correctly with positive number', () => {
        expect(testPackage.value).to.equal(10);
        testPackage.value = 5;
        expect(testPackage.value).to.equal(5);
    });
    it('value setter sets the new value correctly with zero', () => {
        expect(testPackage.value).to.equal(10);
        testPackage.value = 0;
        expect(testPackage.value).to.equal(0);
    });
    ...
    it('VAT setter sets the new value correctly', () => {
        expect(testPackage.VAT).to.equal(20);
        testPackage.VAT = 5;
        expect(testPackage.VAT).to.equal(5);
    });
    ...
    it('active status setter sets the new status correctly', () => {
        expect(testPackage.active).to.be.true;
        testPackage.active = false;
        expect(testPackage.active).to.be.false;
    });
    it('toString() method returns the correct result with active package 20% VAT', () => {
        const expected = [
            `Package: test`,
